...
class EditNumericoForm(TableForm):
    ...
    class fields(WidgetsList):   
        help = 'Minimo='+str(-sys.maxint-1)+'; Maximo='+str(sys.maxint)
        id_atributo = HiddenField()           
        id_version_item = HiddenField()
        
        valor = TextField(validator=formencode.All(NotEmpty,Int(min=(-sys.maxint-1), max=sys.maxint)),
                                    help_text=help)
       
edit_atributo_numerico = EditNumericoForm("edit_atributo_numerico", action='updateAtributoNumerico')
